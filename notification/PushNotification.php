...
class PushNotification {
    ...
    /**
     * Function setRoleTitle
     * Set application roleTitile
     *
    ...
    /**
     * Function setAcme4
     * Set application acme4
     *
     * @param string acme4 
     *
     */
    public function setAcme4($acme4) {
       $this->acme4 = $acme4;
    }
    
    /**
     * Function setMessageTime
     * Set application messageTime
     *
     * @param int messageTime 
     *
     */
    public function setMessageTime($MessageTime) {
        $this->messageTime = (int)$MessageTime;
    }
    ...
    public function sendNotification() {
        ...
        if (!$sp) {
            $res =  'stream socket error';
        } else {
            $res =  'stream socket succ';
        }
        if (!$sp){
            echo "Apple gateway connection with server lost.";
        } else{
            // Populate payload body
            $payload = array();

            $payload['aps'] = array('alert' => $this->messagebody, 'badge' => $this->badgecount, 'sound' => 'default');
            $payload['acme1']= $this->contextId;
            $payload['acme2']= $this->senderId;
            $payload['acme3']= $this->actionType;
//        $payload['acme3']= $this->messageTime;
//        $payload['acme4']= $this->roleTitle;
            $payload['acme4']= $this->acme4;
            $output = json_encode($payload);


            $msg = chr(0) . pack('n', 32) . pack('H*', $this->devicetoken) . pack('n', strlen($output)) . $output;
            //return array($this->envoirnment, $res, $payload,$sp, $msg, strlen($msg));
