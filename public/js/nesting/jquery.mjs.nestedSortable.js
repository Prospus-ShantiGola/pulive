...
(function($) {
	...
	$.widget("mjs.nestedSortable", $.extend({}, $.ui.sortable.prototype, {
				...
				if(itemElement != this.currentItem[0] //cannot intersect with itself
					...
					if (o.isTree && $(itemElement).hasClass(o.collapsedClass) && o.expandOnHover) {
						...
						if (!this.hovering) {
							...
							$(itemElement).find('.node-left').dblclick();
							var self = this;
							this.hovering = window.setTimeout(function() {
								$(itemElement).addClass(o.expandedClass);
			...
			if (parentItem != null && nextItem == null &&
				...
				if (o.isTree && parentItem.children(o.listItem).children('li:visible:not(.ui-sortable-helper)').length < 1) {
					parentItem.removeClass(this.options.branchClass + ' ' + this.options.expandedClass)
							  .addClass(this.options.leafClass).removeClass('isChild');
		...
		serialize: function(options) {
			...
			$(items).each(function() {
				var res = ($(o.item || this).attr(o.attribute || 'data-id') || '')
						.match(o.expression || (/(.+)[-=_](.+)/)),
				    pid = ($(o.item || this).parent(o.listType)
						.parent(o.items)
						.attr(o.attribute || 'data-id') || '')
						.match(o.expression || (/(.+)[-=_](.+)/));
		...
		toHierarchy: function(options) {
			...
			$('*[class*="changedNode"]').each(function(i,v){$(v).removeClass('changedNode'+i)})
			count= 0;
			var o = $.extend({}, this.options, options),
				sDepth = o.startDepthCount || 0,
			    ret = [];

			$(this.element).children(o.items).each(function (i,v) {
			...
			function _recursiveItems(item) {
				var id = ($(item).attr(o.attribute || 'data-id') || '').match(o.expression || (/(.+)[-=_](.+)/));
				if (id) {
					var currentItem = {"id" : id[2]};
					var key;
					var currentItemArr=[];
					$(item).find('.node-content').first().find('input').each(function(i,v){
						...
						currentItem[key] = $(v).val();


					});
					
					$(item).find('.node-content').first().find('input[type=text]').each(function(i,v){
						$(v).addClass('changedNode' +count);
						currentItem['changedElement'] = 'changedNode'+count;
						count=count+1;
						if($(v).attr('inputchange')==undefined){
							currentItem['inputchange'] = 0;
						}
						else{
							currentItem['inputchange'] = $(v).attr('inputchange');	
						}
					...
					if ($(item).children(o.listType).children(o.items).length > 0) {
						...
						$(item).children(o.listType).children(o.items).each(function(childKey,childVal) {
							var level = _recursiveItems(childVal);
		...
		toArray: function(options) {
			...
			function _recursiveArray(item, depth, left) {
				...
				id = ($(item).attr(o.attribute || 'data-id')).match(o.expression || (/(.+)[-=_](.+)/));
		...
		_clearEmpty: function(item) {
			...
			var checkCLassEmpty = $(item).hasClass('isChild');
			if (emptyList.length && !emptyList.children().length && !o.doNotClear && !checkCLassEmpty) {
				o.isTree && $(item).removeClass(o.branchClass + ' ' + o.expandedClass).addClass(o.leafClass);
				//emptyList.remove();
			} else if (o.isTree && emptyList.length && emptyList.children().length && emptyList.is(':visible') && !checkCLassEmpty) {
